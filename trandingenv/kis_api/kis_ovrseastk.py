...
def get_overseas_order(ord_dv="", excg_cd="", itm_no="", qty=0, unpr=0, tr_cont="", FK100="", NK100="", dataframe=None):  # 국내주식주문 > 주식주문(현금)
    ...
    if ord_dv == "buy":
        if excg_cd in ("NASD","NYSE","AMEX"):
            tr_id = "VTTT1002U"  # 미국 매수 주문 [모의투자] VTTT1002U / 실제 투자 TTTT1002U
        elif excg_cd == "SHEK":
            tr_id = "VTTS1002U"  # 홍콩 매수 주문 [모의투자] VTTS1002U / 실제 투자 TTTT1002U?
        elif excg_cd == "SHAA":
            tr_id = "VTTS0202U"  # 중국상해 매수 주문 [모의투자] VTTS0202U / 실제 투자 TTTS0202U
        elif excg_cd == "SZAA":
            tr_id = "VTTS0305U"  # 중국심천 매수 주문 [모의투자] VTTS0305U / 실제 투자 TTTS0305U
        elif excg_cd == "TKSE":
            tr_id = "VTTS0308U"  # 일본 매수 주문 [모의투자] VTTS0308U / 실제 투자 TTTS0308U
        elif excg_cd in ("HASE", "VNSE"):
            tr_id = "VTTS0311U"  # 베트남(하노이,호치민) 매수 주문 [모의투자] VTTS0311U / 실제 투자 TTTS0311U
        else:
            print("해외거래소코드 확인요망!!!")
            return None
    elif ord_dv == "sell":
        if excg_cd in ("NASD", "NYSE", "AMEX"):
            tr_id = "VTTT1006U"  # 미국 매도 주문 [모의투자] VTTT1006U / 실제투자 TTTT1006U
        elif excg_cd == "SHEK":
            tr_id = "VTTS1001U"  # 홍콩 매도 주문 [모의투자] VTTS1001U / 실제투자 TTTS1001U
        elif excg_cd == "SHAA":
            tr_id = "VTTS1005U"  # 중국상해 매도 주문 [모의투자] VTTS1005U / 실제투자 TTTS1005U
        elif excg_cd == "SZAA":
            tr_id = "VTTS0304U"  # 중국심천 매도 주문 [모의투자] VTTS0304U / 실제투자 TTTS0304U
        elif excg_cd == "TKSE":
            tr_id = "VTTS0307U"  # 일본 매도 주문 [모의투자] VTTS0307U / 실제투자 TTTS0307U
        elif excg_cd in ("HASE", "VNSE"):
            tr_id = "VTTS0311U"  # 베트남(하노이,호치민) 매도 주문 [모의투자] VTTS0311U / 실제투자 TTTS0310U
        else:
            print("해외거래소코드 확인요망!!!")
            return None
    else:
...
#해외주식 시장가주문 함수
def get_market_price_order(ord_dv="", excg_cd="", itm_no="", qty=0, unpr=0, tr_cont="", FK100="", NK100="", dataframe=None):  # 국내주식주문 > 주식주문(현금)
    url = '/uapi/overseas-stock/v1/trading/order'
...
def get_overseas_inquire_balance(excg_cd="", crcy_cd="", tr_cont="", FK100="", NK100="", dataframe=None):
    ...
    tr_id = "VTTS3012R"   # 모의투자 VTTS3012R 실재투자 TTTS3012R
...
def get_overseas_inquire_balance_lst(excg_cd="", crcy_cd="", tr_cont="", FK100="", NK100="", dataframe=None):
    url = '/uapi/overseas-stock/v1/trading/inquire-balance'
    tr_id = "VTTS3012R"   # 모의투자 VTTS3012R / TTTS3012R
...
def get_overseas_inquire_present_balance(dv="03", dvsn="01", natn="000", mkt="00", inqr_dvsn="00", tr_cont="", FK100="", NK100="", dataframe=None):
    ...
    tr_id = "VTRP6504R"   # 모의투자 VTRP6504R / CTRP6504R
...
# Input: None (Option) 상세 Input값 변경이 필요한 경우 API문서 참조
# Output: DataFrame (Option) output API 문서 참조 등
def get_overseas_inquire_psamount(mrk="NASD", price="230.0000", item="AAPL", tr_cont="", dataframe=None):
    url = '/uapi/overseas-stock/v1/trading/inquire-psamount'
    tr_id = "VTTS3007R"   # 모의투자 VTTS3007R 실제투자 TTTS3007R
